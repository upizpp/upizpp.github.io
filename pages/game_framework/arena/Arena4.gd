...
# 更新碰撞箱的信息
func init_shape():
	static_body.get_child(0).position = Vector2(0.0, -(current_size.y * 0.5 + Thickness * 0.5))
	static_body.get_child(0).scale = Vector2(current_size.x + Thickness * 2.0, Thickness)
	static_body.get_child(1).position = Vector2(0.0, current_size.y * 0.5 + Thickness * 0.5)
	static_body.get_child(1).scale = Vector2(current_size.x + Thickness * 2.0, Thickness)
	static_body.get_child(2).position = Vector2(-(current_size.x * 0.5 + Thickness * 0.5), 0.0)
	static_body.get_child(2).scale = Vector2(Thickness, current_size.y)
	static_body.get_child(3).position = Vector2(current_size.x * 0.5 + Thickness * 0.5, 0.0)
	static_body.get_child(3).scale = Vector2(Thickness, current_size.y)
